...
public class DedicatedServerNetwork implements ServerNetwork {
    ...
    private void sendPacketToAddress(Packet packet, SocketAddress address) {
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            ...
            System.err.println("Failed to encode/send packet: " + e.getMessage());
        }
    }
    ...
    private void handleIncomingPacket(DatagramPacket datagram) {
        try (DataInputStream dis = new DataInputStream(new ByteArrayInputStream(datagram.getData(), 0, datagram.getLength()))) {
            ...
            System.err.println("Failed to decode incoming packet: " + e.getMessage());
        }
    }
